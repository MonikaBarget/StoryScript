...
export const useStateStore = defineStore('appState', () => {
    ...
    // Initialize game as an empty IGame object
    const game = ref<IGame>({} as IGame);
    ...
    const setStoreData = (factory: ServiceFactory) => {
        ...
        // Initialize currentDescriptionHtml if it doesn't exist
        if (!game.value.currentDescriptionHtml) {
            game.value.currentDescriptionHtml = "";
        }
    }
